...
const EditMinutesWithAISchema = z.object({
  companyId: z.string().min(1, "Company ID é obrigatório"),
  editType: z.enum([
    "improve",           // Melhorar texto e linguagem
    "correct",           // Corrigir erros gramaticais e ortográficos
    "expand",            // Expandir seções específicas
    "summarize",         // Resumir seções muito longas
    "add_section",       // Adicionar nova seção
    "reformat",          // Reformatar estrutura
    "custom"             // Edição personalizada com instruções
  ]).default("improve"),
  instructions: z.string().optional().describe("Instruções específicas para a edição (obrigatório se editType for 'custom')"),
  targetSections: z.array(z.string()).optional().describe("Seções específicas para editar (ex: ['deliberacoes', 'votacoes'])"),
  preserveFormat: z.boolean().default(true).describe("Preservar formatação original"),
...
// PUT /minutes/:id/edit - Editar ATA com IA
minutes.put("/:id/edit", async (c) => {
  try {
    const minuteId = c.req.param("id");
    const body = await c.req.json();
    const validation = EditMinutesWithAISchema.safeParse(body);
    ...
    // Validar instruções se editType for 'custom'
    if (validation.data.editType === "custom" && !validation.data.instructions) {
      return c.json(
        {
          success: false,
          error: "Instruções são obrigatórias quando editType é 'custom'",
        },
        400
      );
    }
    ...
    const context = extractRequestContext(c, validation.data);
    const result = await minutesService.editMinutesWithAI(
      minuteId,
      validation.data,
      context
    );
    ...
    console.error("Error in /minutes/:id/edit:", error);
    return c.json(
      {
        success: false,
        error: error instanceof Error ? error.message : "Erro interno",
